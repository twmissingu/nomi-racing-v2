...
## Bristol Short Track — 0.9 km tight oval with 28° progressive banking, 14m wide road.
## Colosseum-style with concrete walls close to the racing surface.

# --- Track geometry constants ---
const TURN_RADIUS: float = 40.0
const STRAIGHT_LENGTH: float = 200.0
const HALF_STRAIGHT: float = 100.0
const ROAD_WIDTH: float = 14.0
const BANK_ANGLE: float = 0.35  # ~20 degrees (reduced for stability)
const NUM_SEGMENTS: int = 192
const BARRIER_HEIGHT: float = 3.0
const BARRIER_WIDTH: float = 0.6
...
func _ready() -> void:
	...
	_build_ai_path()
	_build_environment()
	_build_colosseum_walls(points)

func get_spawn_transform(index: int) -> Transform3D:
	var stagger_z: float = -10.0 - float(index) * 7.0
	var side_offset: float = 2.5 if index % 2 == 1 else -2.5
...
func _compute_point(s: float) -> Dictionary:
	...
	if s < s1:
		...
		# Progressive banking — increases through the turn
		bank = BANK_ANGLE * _progressive_bank(t_norm)
...
func _progressive_bank(t: float) -> float:
	# Gradual progressive banking: slow ramp in/out prevents spin
...
func _build_barrier_wall(points: Array[Dictionary], side: float, wall_name: String) -> void:
	...
	var up := Vector3.UP
	var color_concrete := Color(0.8, 0.78, 0.75)
...
func _build_ground() -> void:
	...
	ground_mesh.size = Vector3(300, 0.05, 400)
...
# --- Colosseum walls (steep surrounding walls) ---

func _build_colosseum_walls(points: Array[Dictionary]) -> void:
	# Tall outer walls to give colosseum feel
	var wall_mat := StandardMaterial3D.new()
	wall_mat.albedo_color = Color(0.6, 0.58, 0.55)
	wall_mat.roughness = 0.9

	var half_w: float = ROAD_WIDTH / 2.0 + BARRIER_WIDTH + 1.0
	var wall_height: float = 12.0

	# Upper wall sections — just visual CSG boxes at intervals
	var spacing: int = points.size() / 24
	for i in range(0, points.size(), spacing):
		var p: Dictionary = points[i]
		var right: Vector3 = _get_banked_right(p)

		var wall := CSGBox3D.new()
		var seg_len: float = perimeter / 24.0
		wall.size = Vector3(1.0, wall_height, seg_len * 0.9)
		var wall_pos: Vector3 = p.position + right * (half_w + 0.5) + Vector3.UP * (BARRIER_HEIGHT + wall_height * 0.5)
		...
		var fwd: Vector3 = p.forward
		if fwd.length() > 0.001:
			wall.transform = Transform3D(_basis_facing(fwd), wall_pos)
